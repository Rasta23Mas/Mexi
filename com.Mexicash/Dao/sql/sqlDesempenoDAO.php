...
class sqlDesempenoDAO
{
    ...
    /*//Generar Desempeño
    public function generarDes($tipeFormulario, $descuento, $contrato, $idEstatusArt, $token,
                        ...
                        if ($ps->execute()) {
                            if (empty($token)) {
                                ...
                                if ($ps = $this->conexion->prepare($insertaBitacora)) {
                                    ...
                                    if ($ps->execute()) {
                                        $updateToken = "UPDATE cat_token SET estatus = 2 WHERE id_token =$token";
                                        if ($ps = $this->conexion->prepare($updateToken)) {
                                            if ($ps->execute()) {
                                                $verdad = mysqli_stmt_affected_rows($ps);
                                            } else {
    ...
    //Generar Desempeño
   /* public function generarDesSinInteres($tipeFormulario, $saldoPendiente, $descuentoFinal, $contrato, $idEstatusArt, $estatusAnterior)
    {
        // TODO: Implement guardaCiente() method.
        try {
            $fechaModificacion = date('Y-m-d H:i:s');
